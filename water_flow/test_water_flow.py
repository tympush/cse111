...
from water_flow import water_column_height, pressure_gain_from_water_height, pressure_loss_from_pipe, pressure_loss_from_fittings, reynolds_number, pressure_loss_from_pipe_reduction, convert_to_psi
...
def test_convert_to_psi():
    ...
    assert convert_to_psi(158.7) == approx(23.017,abs=0.001)
    assert convert_to_psi(0) == approx(0,abs=0.001)
    assert convert_to_psi(-100) == approx(-14.504,abs=0.001)
